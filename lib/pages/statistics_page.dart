  ...
  Future<void> _loadAllStats() async {
    ...
    try {
      final results = await Future.wait([
        _apiClient.getDailyStats(),
        _apiClient.getWeeklyStats(),
        _apiClient.getMonthlyStats(),
      ]);

      setState(() {
        _dailyStats = DailyStats.fromJson(results[0]['data']);
        _weeklyStats = WeeklyStats.fromJson(results[1]['data']);
        _monthlyStats = MonthlyStats.fromJson(results[2]['data']);
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
...
class _WeeklyStatsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (stats == null) {
      return const Center(
        child: Text('No weekly statistics available'),
      );
    }
